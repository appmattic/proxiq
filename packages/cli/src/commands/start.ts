...
import { removePidFile, writePidFile } from "../utils/pid.js";
  ...
  program
    ...
    .action(
      async (opts: {
        ...
        if (opts.detach) {
          console.error(
            "--detach is not yet implemented. Run with pm2, systemd, or Docker."
          );
          process.exit(1);
        }
        ...
        const needsEmbedder =
          config.cache.semantic.enabled || config.memory.enabled;
        ...
        middlewareRegistry.register(
          createLoggerMiddleware(logger, config.logging.includePrompts)
        );
        ...
        if (config.auth.required) {
          logger.info(
            ...
            "[auth] required mode — tokens loaded"
          );
        } else if (config.auth.tokens.length > 0) {
          logger.info(
            { tokenCount: config.auth.tokens.length },
            "[auth] optional mode — tokens loaded"
          );
        }
        ...
        setInterval(() => {
          ...
          if (deleted > 0)
            logger.debug({ deleted }, "[cache] purged expired entries");
          const sessionsPurged = memory.purgeExpired();
          if (sessionsPurged > 0)
            logger.debug(
              { sessionsPurged },
              "[memory] purged expired sessions"
            );
          const logsPurged = purgeOldLogs(db, config.retention.logsDays);
          if (logsPurged > 0)
            logger.debug({ logsPurged }, "[retention] purged old request logs");
        }, PURGE_INTERVAL_MS);
      }
    );
}
...
function printBanner(
  config: import("@proxiq/core").Config,
  address: string
): void {
