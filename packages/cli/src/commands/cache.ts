...
export function registerCache(program: Command): void {
  const cacheCmd = program.command("cache").description("Manage the Proxiq cache");
  ...
  cacheCmd
    ...
    .action(async (opts: { port: string }) => {
      ...
      try {
        const res = await fetch(`http://127.0.0.1:${port}/proxiq/cache/clear`, { method: "POST" });
        if (res.ok) {
  ...
  cacheCmd
    ...
    .action(async (opts: { hash: string; port: string }) => {
      const port = parseInt(opts.port, 10);
      try {
        const res = await fetch(`http://127.0.0.1:${port}/proxiq/cache/${opts.hash}`);
        if (res.ok) {
