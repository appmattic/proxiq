...
describe("resolveProvider", () => {
  ...
  it("auto-detects anthropic from sk-ant- prefix", () => {
    expect(resolveProvider(undefined, "", "sk-ant-abc123", DEFAULT_CONFIG)).toBe("anthropic");
  });

  it("auto-detects groq from gsk_ prefix", () => {
    expect(resolveProvider(undefined, "", "gsk_test", DEFAULT_CONFIG)).toBe("groq");
  });

  it("falls back to config default", () => {
    expect(resolveProvider(undefined, "", "", DEFAULT_CONFIG)).toBe("anthropic");
  });
});

describe("buildForwardHeaders", () => {
  it("converts Bearer to x-api-key for anthropic", () => {
    const headers = buildForwardHeaders({ authorization: "Bearer sk-ant-abc" }, "anthropic");
    expect(headers["x-api-key"]).toBe("sk-ant-abc");
    expect(headers["authorization"]).toBeUndefined();
  });

  it("keeps Authorization for non-anthropic providers", () => {
    const headers = buildForwardHeaders({ authorization: "Bearer sk-openai-abc" }, "openai");
