...
server.tool(
  "proxiq_status",
  "Get Proxiq gateway health and status",
  {},
  async () => {
...
server.tool(
  "proxiq_metrics",
  "Get Proxiq cache metrics and token savings",
  {},
  async () => {
...
server.tool(
  ...
  "Send an LLM completion request through Proxiq (supports all providers)",
  {
    provider: z
      .string()
      .default("anthropic")
      .describe("Provider: anthropic, openai, groq, etc."),
...
server.tool(
  ...
  "Classify a prompt's complexity tier (simple / standard / complex)",
  {
    ...
    api_key: z
      .string()
      .optional()
      .describe("Anthropic API key for the classifier"),
  },
  async (params) => {
    ...
    const body = {
      ...
      max_tokens: 10,
      system:
        "Classify this prompt as: simple, standard, or complex. Reply with ONLY one word.",
...
server.tool(
  ...
  {
    period: z
      .enum(["today", "daily_avg", "weekly", "monthly"])
      .default("today")
      .describe(
        "Time period: today, daily_avg (30-day avg), weekly (last 7d), monthly (last 30d)"
      ),
    user: z
      .string()
      .optional()
      .describe(
        "Filter stats by user label (e.g. 'alice'). Omit for all users."
      ),
  },
  async (params) => {
    ...
    if (cost) {
      lines.push(
        `Cost: $${cost.actualUsd?.toFixed(4)} actual | $${cost.wouldHaveCostUsd?.toFixed(4)} without routing`
      );
